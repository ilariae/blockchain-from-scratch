...
impl StateMachine for LightSwitch {
    ...
    fn next_state(starting_state: &bool, t: &()) -> bool {
        !starting_state // invert switch state
    }
}
    ...
    fn next_state(starting_state: &TwoSwitches, t: &Toggle) -> TwoSwitches {
        match t {
            Toggle::FirstSwitch => TwoSwitches { 
                first_switch: !starting_state.first_switch, // toggle first switch
                second_switch: if starting_state.first_switch { // if first off
                    false // also turn second off 
                } else {
                    starting_state.second_switch // otherwise keep second unchanged 
                } , 
            },
            Toggle::SecondSwitch => TwoSwitches { 
                first_switch: starting_state.first_switch, 
                second_switch: !starting_state.second_switch, // toggle second switch 
            },
        }
    }
}
